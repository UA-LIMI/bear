...
export const RoomControlPanel: React.FC<RoomControlPanelProps> = ({
  ...
  useEffect(() => {
    ...
    if (mqttConnected && rooms.length > 0) {
      rooms.forEach(room => {
        subscribeToRoomStatus(room.roomNumber, (status: any) => {
  ...
  return <div className="h-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[calc(100vh-180px)]">
        <div className="lg:col-span-1 overflow-y-auto pr-2 space-y-4">
          {filteredRooms.map((room: any) => <RoomCard key={room.roomNumber} room={room} isSelected={selectedRoom && selectedRoom.roomNumber === room.roomNumber} onClick={() => setSelectedRoom(room)} />)}
        </div>
