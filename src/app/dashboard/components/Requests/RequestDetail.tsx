...
export const RequestDetail: React.FC<RequestDetailProps> = ({
  ...
  const [selectedStaff, setSelectedStaff] = useState<any>(request.assignedStaff || null);
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [aiAnalysis, setAiAnalysis] = useState<any>(null);
  // Filter staff by department relevant to the request type
  const getRelevantDepartment = (type: string) => {
    switch (type) {
      case 'food':
      case 'room-service':
  ...
  return <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 h-full flex flex-col">
      <div className="p-6 border-b border-gray-200 dark:border-gray-700">
        ...
        {showStaffAssignment && <div className="mt-4 bg-blue-50 dark:bg-blue-900/10 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 mb-3">
              {availableStaff.map((staff: any) => <div key={staff.id} onClick={() => setSelectedStaff(staff.name)} className={`p-2 rounded-lg border cursor-pointer ${selectedStaff === staff.name ? 'border-blue-500 bg-blue-100 dark:bg-blue-900/30' : 'border-gray-200 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-blue-900/10'}`}>
                  <div className="flex items-center">
      ...
      <div className="p-6 flex-grow overflow-y-auto">
        ...
        <div className="space-y-4">
          {request.conversation.map((message: any, index: number) => <div key={index} className={`flex ${message.sender === 'guest' ? 'justify-start' : 'justify-end'}`}>
